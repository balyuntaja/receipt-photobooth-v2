...
/**
 * Thermal Printer Utilities (USB + BLE) - STABLE VERSION
 * Optimized for ESC/POS printers (XS-80, 58mm)
 */
...
export async function connectPrinterUSB(vendorId = 0x0418) {
  ...
  console.log('[Printer] USB connected:', device.productName);
}
...
export async function connectPrinterBLE() {
  ...
  for (const svc of services) {
    const chars = await svc.getCharacteristics();
    for (const c of chars) {
      if (!c.properties.write) continue;
      if (c.uuid.toLowerCase() === NORDIC_UART_TX) {
        writeChar = c;
        break;
      }
...
/* =========================
   IMAGE → ESC/POS
========================= */
async function convertToESCPos(imageDataUrl, width = 384) {
  return new Promise((resolve, reject) => {
    const img = new Image();
      ...
      try {
        ...
        for (let y = 0; y < height; y++) {
          for (let x = 0; x < width; x++) {
            ...
            bitmap[y * bytesPerRow + (x >> 3)] |= bit << (7 - (x % 8));
          }
        }
        ...
        cmd.push(height & 0xff, (height >> 8) & 0xff);
...
async function sendToPrinter(data) {
  ...
  // BLE
  if (PRINTER_STATE.type === 'bluetooth') {
    const char = PRINTER_STATE.characteristic;

    for (let i = 0; i < data.length; i += BLE_CHUNK_SIZE) {
      const chunk = data.slice(i, i + BLE_CHUNK_SIZE);

      for (let retry = 0; retry < BLE_RETRY; retry++) {
        try {
          await char.writeValue(chunk); // 🔥 stable
          break;
        } catch (e) {
          if (retry === BLE_RETRY - 1) throw e;
...
/* =========================
   PRINT
========================= */
export async function printPhotostrip(imageDataUrl, qty = 1) {
  if (!PRINTER_STATE.device) {
    throw new Error('Printer belum connect');
  }

  const data = await convertToESCPos(imageDataUrl);
